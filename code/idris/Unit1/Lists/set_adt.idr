...
this file will show that Idris finds many remaining "holes." These are
...
-- function. Separating this code allows us to see and to emphasize the
...
combine nil l2 = l2
combine (h1::t1) (l2) = ite (member h1 l2)
                            (combine t1 l2)
                            (h1 :: (combine t1 l2))
--append each element as you go
...
list_difference l nil= l
list_difference (h::t) l = ite (member h l)
                               (list_difference t l)
                               (h::(list_difference t l))

--elements that are in first but not in second
--so remove elements that are in second but not in 1
-- Return the set difference, s1 minus s2
--set_difference: (eq a) => (s1: set a) -> (s2: set a) -> set a
...
set_exists p (mkSet (h::t)) = ite (p h)
                                  (true)
                                  (set_exists p (mkSet t))
-------------------------------------
...
set_witness p (mkSet (h::t)) = list_witness p (h::t)
...
--pair elt takes an elt h and a list b and returns a list of all pairs
--(h, e) where e is an elt of list b
pairelt: a -> list b -> list (pair a b)
pairelt someA nil = nil
pairelt someA (b::tb)= (mkPair someA b) :: (pairelt someA tb)

--given a list of a and a list of b return a list of all ab pairs
pairlist :list a-> list b-> list (pair a b)
pairlist nil lb = nil
pairlist (h::t) lb = (pairelt h lb) ++ (pairlist t lb)
--avoidmaking list of lists, need to flatten so append
